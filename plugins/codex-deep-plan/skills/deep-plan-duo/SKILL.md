...
5. Root causes, not symptoms. A chain terminates in a violated invariant, missing abstraction,
   wrong domain model, broken contract or absent constraint. "The code was wrong" is stopping early.
...
7. Real ≠ large. Prefer the narrowest change that eliminates the cause; blast radius and
   reversibility are scored as counterweights (`references/solution-rubric.md`).
...
| Phase 4 start | `templates/PLAN.md`, `references/phases.md` §4 |
...
| Phase 8 | `templates/PLAN.md` or `templates/DECISION-REQUIRED.md` (re-read; do not reconstruct) |
...
| # | Phase | Artifact | Gate |
|---|---|---|---|
| 0 | Scope | `00-scope.md`, `meta.json`, `inputs/*.md`, `debate/r0-prompt.md` | inputs verbatim, base SHA pinned, closure criteria observable, brief leak-free |
| 1 | Evidence | `01-evidence.md` | both linters OK; blocking unknowns listed |
| 2 | Root cause | `02-root-cause.md` | every chain ends in a cause class; shared-cause verdict |
| 3 | Designs | `03-designs.md` | ≥3 candidates incl. do-nothing, largest, tempting workaround; rubric-scored |
| 4 | Draft plan | `04-plan-draft.md` | every decision cites F-/V- ids; then `01`–`04` → mode 000 |
| 5 | Codex round 0 | `debate/r0-codex.{json,md}` + sidecars | blind, `--fresh`, validated with `--repo`; or SKIPPED |
| 6 | Divergence | `debate/divergence.md` | every difference is a `D-nn` with a question |
| 7 | Rounds 1..N | `05-disagreements.md`, `debate/rN-*` | termination condition recorded |
| 8 | Final | `PLAN.md` or `DECISION-REQUIRED.md` | linters OK, completion gate, plan-mode handoff |

Detail per phase is in `references/phases.md`. Track one todo per phase.
...
Before finishing confirm: both linters exit 0 on the artifact dir and `PLAN.md`; every input
maps to root cause → change → test → closure criterion; every test in the matrix states why it
...
review status, "evidence in 01-evidence.md, debate in debate/") followed by `PLAN.md` verbatim —
never a summary. Then call `ExitPlanMode`. If the outcome is `DECISION-REQUIRED.md`, write that
instead with line 1 saying approval is not yet possible. If no plan file path is provided, print
