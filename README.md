...
| Backend | `--via codex` (default) or `--via ccr:<alias>`. CCR ≥ 0.6.0 owns detached jobs, status, and cancellation. The runner uses plan permissions, empty strict MCP configuration, and disallowed edit tools. A required smoke binds the alias, generated model identity, version, and launch digest. Later CCR rounds resume the participant session with an explicit expected parent job, resolved before prompt preparation. Submission tokens recover lost receipts without duplicate work. Unknown admission or stop evidence never permits automatic relaunch. |
