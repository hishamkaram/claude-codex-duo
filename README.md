...
| Backend | `--via codex` (default) or `--via ccr:<alias>`. The ccr backend passes the configured alias only to `ccr launch --model <alias>` **before** `--`; child Claude Code options follow `--`. `provider_model` is descriptive, never a CLI selector. Its plan-mode, empty strict-MCP, write-tool-disallowed child runs in its own process group (`pgid=`). `--probe --via ccr:<alias>` runs one fail-closed read-only smoke per alias per run; its record binds the alias, provider model, CCR-generated `claude_model_id`, observed child `system/init.model`, CCR version and launch digest. The runner accepts completion only if observed child ID equals `claude_model_id`; mismatch/unrecognized routes exit 4 without another alias or Claude fallback. Same sidecars, same exit codes, same `--write` refusal. |

The probe has three results, not two: `PROBE SUCCEEDED` (exit 0), `PROBE UNAVAILABLE` (exit 1, an authoritative negative about the launch path), and `PROBE UNDETERMINED` (exit 0) — the companion could not determine authentication, which never refuses a run: record it and proceed to the launch, which settles it.
...
| Runner exit `6` (DETACHED) | The watch bound elapsed while the job was still running. Nothing was cancelled and no work was lost: `--max-min` bounds the watcher, not the job. Run the `attach_command` the runner printed to resume the watch and collect the result; never relaunch, which would start a second job. `phase-gate.sh release` refuses it while the job is not provably finished; once it provably is (an empty process group, or the companion reporting the job over), release works and rotates the claim, which is the escape when the attach cannot reach its backend. Note that a successful attach writes `.exit`, and `release` refuses a finished attempt — after an attach, the next launch gate rotates the claim itself. |
