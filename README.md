...
| Runner exit `3` (TIMEOUT) | The watch bound elapsed and the job was confirmed gone. Large diffs should be split by subsystem in Phase 0. |
| Runner exit `6` (DETACHED) | The watch bound elapsed while the job was still running. Nothing was cancelled and no work was lost: `--max-min` bounds the watcher, not the job. Run the `attach_command` the runner printed to resume the watch and collect the result; never relaunch, which would start a second job. `phase-gate.sh release` refuses a detached prefix until an attach publishes an outcome. |
