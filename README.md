...
| Backend | `--via codex` (default) or `--via ccr:<alias>`. The ccr backend runs `ccr launch --model <alias> --permission-mode plan` with an empty strict MCP config and the write tools disallowed, in its own process group (recorded as `pgid=` on the launch line so a stall can kill the whole group and `phase-gate.sh release` can prove it is gone); `--fresh`/`--resume-last` map to a new session / `--resume <session_id>` (`.ccr-last-session`), `--resume-session <id>` names one; `--probe --via ccr:<alias>` gates on the ccr version, records `ccr model show --json` and runs a read-only smoke in a throwaway repository, and every ccr launch in a run directory requires that smoke record (`.ccr-smoke.<alias>`) for its exact launch line — missing or violated means exit 4 with nothing launched. Same sidecars, same exit codes, same `--write` refusal. |

Exit codes (identical for both backends): `0` completed, `1` failed, `2` stalled, `3` timeout, `4` launch error or invalid invocation (nothing is written when the runner cannot take its launch claim — missing, replaced or already-taken claim, or a bad argument on a prefix that has a claim; a launch error after the claim is taken records its sidecars; a ccr alias without its smoke record), `5` stalled or timed out with the cancel unconfirmed (for ccr: a process in the recorded group survived TERM and KILL). A bad command line never exits 1, so a typo cannot be mistaken for an upstream failure, and `5` means do not retry because a worker may still be running.
